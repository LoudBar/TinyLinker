...
object UrlShrinkService {
  ...
  def shrinkUrl(originalUrl: String)(implicit ec: ExecutionContext): Future[String] = {
    val key = StringEncoder.generateUniqueKey(originalUrl)
    val shortenedUrl = s"http://localhost:8080/${StringEncoder.base62Encode(key)}"
...
object StringEncoder {
  ...
  def generateUniqueKey(originalUrl: String): Long = {
  ...
  def base62Encode(number: Long): String = {
